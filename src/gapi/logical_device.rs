...
use crate::gapi::instance::{Instance, Layer, PORTABILITY_MACOS_VERSION};
...
impl LogicalDevice {
    ...
    pub fn new(
        ...
        let family_infos =
            Self::find_queue_families(instance, surface, &physical_device, requests)?;
        ...
        let device = unsafe {
            instance
                .get()
                .create_device(*physical_device.get(), &create_info, None)
        }
